...
Future<Map<String, dynamic>> agentChatWithContext({
  required int userId,
  required String message,
  List<Map<String, dynamic>> calendarEvents = const [],
...
// ── SMS Intelligence Confirmation ────────────────────────────────

Future<Map<String, dynamic>> confirmSmsTask({
  required int userId,
  required Map<String, dynamic> extraction,
}) async {
  try {
    final response = await http.post(
      Uri.parse('$baseUrl/api/sms-intelligence/confirm'),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'user_id': userId,
